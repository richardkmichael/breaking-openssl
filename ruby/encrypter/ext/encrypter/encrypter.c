...
// static VALUE encrypter_encrypt()
{
  ...
  unsigned int encrypted_string_length;
  char *plaintext_string_ptr, *encrypted_string_ptr;
  ...
  plaintext_string_ptr = RSTRING_PTR(rb_string_plaintext);
  printf("DEBUG: plaintext_string: %s\n", plaintext_string_ptr);

  encrypted_string_ptr = encrypt(plaintext_string_ptr, &encrypted_string_length);
  printf("DEBUG: encrypted_string: %s\n", encrypted_string_ptr);

  // return rb_str_new2("Encrypted message.");
  rb_encrypted_string = rb_str_new2(encrypted_string_ptr); // rb_str_new2 initializes from a signed char*.
